...
import utils
import optparse
from sequence.fasta import FastaIterator
...
def main(sysargs=[]):
    ...
    rv=transform(args,outFile=outFile,removeDupSeq=opts.dropDup)
...
def transform(args,outFile=sys.stdout, removeDupSeq=False):
    """This is where the magic happens
    """
    seqs=set([])
    for rec in FastaIterator(args):
        if removeDupSeq:
            if rec.sequence in seqs:
                continue
            else:
                seqs.add(rec.sequence)
        print >> outFile, rec.tabFormat()
