...
#
__version__  =[int(x) for x in "$Revision: 1.46 $".split()[1].split('.')]
...
__all__=["CommandLine","timeprofile",'readArgsOrFiles','flatten','unique',
         ...
         'getIterable','unzip','lastLine','benchmark','wrap']
...
class MultiDictCluster(dict):
    ...
    def eAppend(self,expt,name,eWeight=1.0,pctNorm=False):
...
#        self.refresh_gOrder()
    ...
    def readCdt(self,cdtFile):
        ...
        self.eNames=lines.next().strip().split('\t')[4:]
        lines.next()
        self.eWeights=[float(x) for x in lines.next().strip().split('\t')[4:]]
        for l in lines:
            f = l.strip().split('\t')
            gID=f[1]
            self.gNames[gID]=None
            self.gOrder.append(gID)
            self.gWeights[gID] = float(f[3])
            values = [float(x) for x in f[4:]]
            for i,eName in enumerate(self.eNames):
                if eName not in self:
                    self[eName] = {}
                self[eName][gID]=values[i]
